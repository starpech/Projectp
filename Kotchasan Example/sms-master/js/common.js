...
function countryChanged(prefix) {
  ...
  var _contryChanged = function() {
    if (this.value != "TH") {
      $G($E(prefix + "provinceID").parentNode.parentNode).addClass("hidden");
      $G($E(prefix + "province").parentNode.parentNode).removeClass("hidden");
    } else {
      $G($E(prefix + "provinceID").parentNode.parentNode).removeClass("hidden");
      $G($E(prefix + "province").parentNode.parentNode).addClass("hidden");
    }
  };
  if ($E(prefix + "country")) {
    $G(prefix + "country").addEvent("change", _contryChanged);
    _contryChanged.call($E(prefix + "country"));
  }
}
