...
class Model extends \Kotchasan\Model
{
    ...
    public function importGrade($data)
    {
        $course = array(
            'course_code' => Text::topic($data[$this->header[0]]),
            'year' => (int) $data[$this->header[7]],
            'term' => (int) $data[$this->header[8]],
        );
        $grade = array(
            'number' => (int) $data[$this->header[1]],
            'student_id' => Text::topic($data[$this->header[2]]),
            'midterm' => (int) $data[$this->header[3]],
            'final' => (int) $data[$this->header[4]],
            'grade' => Text::topic($data[$this->header[5]]),
            'room' => (int) $data[$this->header[6]],
        );
    ...
    public function importStudent($data)
    {
        ...
        $user = array(
            ...
            'parent' => Text::topic($data[$this->header[8]]),
            'parent_phone' => Text::topic($data[$this->header[9]]),
        );
