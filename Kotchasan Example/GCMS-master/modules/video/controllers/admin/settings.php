<?php
/**
 * @filesource modules/video/controllers/admin/settings.php
 *
...
class Controller extends \Gcms\Controller
{
    ...
    {
        // ข้อความ title bar
        $this->title = Language::trans('{LNG_Module settings} {LNG_Video}');
        // เลือกเมนู
        $this->menu = 'modules';
        ...
        if ($index && $login && (Gcms::canConfig($login, $index, 'can_config') || !Login::notDemoMode($login))) {
            ...
            // แสดงฟอร์ม
            $section->appendChild(createClass('Video\Admin\Settings\View')->render($request, $index));
