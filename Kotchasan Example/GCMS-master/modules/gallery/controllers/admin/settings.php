<?php
/**
 * @filesource modules/gallery/controllers/admin/settings.php
 *
...
class Controller extends \Gcms\Controller
{
    ...
    {
        // ข้อความ title bar
        $this->title = Language::trans('{LNG_Module settings} {LNG_Gallery}');
        // เลือกเมนู
        $this->menu = 'modules';
        // อ่านข้อมูลโมดูล และ config
        $index = \Index\Adminmodule\Model::getModuleWithConfig('gallery', $request->request('mid')->toInt());
        // admin
        ...
        if ($index && $login && (Gcms::canConfig($login, $index, 'can_config') || !Login::notDemoMode($login))) {
            ...
            // แสดงฟอร์ม
            $section->appendChild(createClass('Gallery\Admin\Settings\View')->render($request, $index));
