<?php
/**
 * @filesource modules/documentation/models/admin/init.php
 *
        ...
        if (!empty($modules) && $login) {
            ...
            foreach ($modules as $item) {
                if (Gcms::canConfig($login, $item, 'can_config') || !Login::notDemoMode($login)) {
                    Gcms::$menu->menus['modules'][$item->module]['config'] = '<a href="index.php?module=documentation-settings&amp;mid='.$item->id.'"><span>{LNG_Config}</span></a>';
                    Gcms::$menu->menus['modules'][$item->module]['category'] = '<a href="index.php?module=documentation-category&amp;mid='.$item->id.'"><span>{LNG_Category}</span></a>';
                }
                if (Gcms::canConfig($login, $item, 'can_write') || !Login::notDemoMode($login)) {
                    Gcms::$menu->menus['modules'][$item->module]['setup'] = '<a href="index.php?module=documentation-setup&amp;mid='.$item->id.'"><span>{LNG_Contents}</span></a>';
