...
class Controller extends \Gcms\Controller
{
    ...
    {
        // ข้อความ title bar
        $this->title = Language::trans('{LNG_Module settings} {LNG_Event}');
        // เลือกเมนู
        $this->menu = 'modules';
        // อ่านข้อมูลโมดูล และ config
        $index = \Index\Adminmodule\Model::getModuleWithConfig('event', $request->request('mid')->toInt());
        // admin
        ...
        if ($index && $login && (Gcms::canConfig($login, $index, 'can_config') || !Login::notDemoMode($login))) {
            ...
            // แสดงฟอร์ม
            $section->appendChild(createClass('Event\Admin\Settings\View')->render($request, $index));
