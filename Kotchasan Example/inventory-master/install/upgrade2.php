    ...
    if (empty($_POST['username']) || empty($_POST['password'])) {
        ...
        if (!$error) {
            ...
            try {
                ...
                if (fieldExists($conn, $table, 'serial')) {
                    $conn->query("ALTER TABLE `$table` CHANGE `serial` `product_no` VARCHAR(150) DEFAULT NULL");
                    $conn->query("ALTER TABLE `$table` CHANGE `product_no` `topic` VARCHAR(64) DEFAULT NULL");
                }
                ...
                $conn->query("ALTER TABLE `$table` CHANGE `unit` `unit` INT(11) NOT NULL DEFAULT 0");
                $content[] = '<li class="correct">ปรับปรุงตาราง `'.$table.'` สำเร็จ</li>';
