...
function dbInvMain(){
  ...
  b.bo_date, 
  b.`bo_SupplierName`, 
  (select c.comp_name from comp as c where c.comp_code= b.`site_no`) as site_name, 
  (select sum(bod.amount) from bo_detail as bod where bod.bo_main_id = b.bo_no) as sumall 
  ,approved
  ,bo_RecieveName
  
  
  FROM `bo_main` as b
  where flag_delete = 0 and b.bo_SupplierID = '{$_SESSION['comp_code']}'
  ";
  $find_date = isset($_GET['findDate'])?$_GET['findDate']:'';
  if($find_date){
  ...
  <div class="container-fluid" align="center">
   ...
   <div class="table-responsive">
    ...
    <thead width="100%" class="thead-light">
                <th ><h5 align="center">เลขที่ใบวางบิล</h5></th>
       <th ><h5 align="center">วันที่วางบิล</h5></th>
       <th ><h5 align="center">บริษัทที่วางบิล</h5></th>
       <th ><h5 align="center">สถานที่รับสินค้า</h5></th>
            ...
            <tbody>
             ...
             foreach($dataInvMain as $row ){
			  ...
			  if($_SESSION['comp_code']!='01') $btnApproved = '';
				   ...
				   <td> {$approved} </td>
           <td>
           <a href='onlineinvoice/bo_pdf.php?id=".$row[0]."' target='_blank' class='btn btn-info btn-sm'><span class='glyphicon glyphicon-edit'></span> PDF </a>
...
$('#find').click(()=>{
  ...
  window.location.href="bo_list_supply.php?findDate="+find_date;
});
/*
$('#printPDF').click(()=>{
  var find_date=$('#findDate').val();
...
$('#find_all').click(()=>{
  window.location.href="bo_list_supply.php";
});
