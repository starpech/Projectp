...
$page_title = "แสดงสถานะรายการขอซื้อเพื่อตรวจสอบการจัดส่ง";
...
<?php 
if($find_date)
  $query = "(SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail01 where comp_code = '{$comp_code}' AND input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail02 where comp_code = '{$comp_code}' AND input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail03 where comp_code = '{$comp_code}' AND input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail04 where comp_code = '{$comp_code}' AND input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail05 where comp_code = '{$comp_code}' AND input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail06 where comp_code = '{$comp_code}' AND input_date = '{$find_date}' order by ISNULL(approve_date))
