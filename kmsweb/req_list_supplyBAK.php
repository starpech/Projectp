...
$page_title = "แสดงสถานะรายการขอซื้อ";
...
<?php 
if($find_date)
  $query = "(SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail01 where comp_code = {$comp_code} && input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail02 where comp_code = {$comp_code} && input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail03 where comp_code = {$comp_code} && input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail04 where comp_code = {$comp_code} && input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail05 where comp_code = {$comp_code} && input_date = '{$find_date}' order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail06 where comp_code = {$comp_code} && input_date = '{$find_date}' order by ISNULL(approve_date))
  " ; //WHERE ISNULL(approve_date)
else 
  $query = "(SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail01 where comp_code = {$comp_code} order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail02 where comp_code = {$comp_code} order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail03 where comp_code = {$comp_code} order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail04 where comp_code = {$comp_code} order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail05 where comp_code = {$comp_code} order by ISNULL(approve_date)) UNION 
            (SELECT *,IF(ISNULL(status),1,status) as 'statusx'  FROM req_detail06 where comp_code = {$comp_code} order by ISNULL(approve_date))
  " ; //WHERE ISNULL(approve_date)
  ...
  <div class="container-fluid" align="center">
		...
		//debug($row);
         ...
         $btnSentProduct = '';
		  if($row['statusx']*1===2){
			...
			$btnSentProduct = " <a href='req_sent.php?id=".$row[0]."' class='btn btn-success btn-sm' > จัดส่ง </a>
			";
		  }
   ...
   ?>
     <tr id="<?php echo $row["id_no"]; ?>" >
	  ...
	  <td>
	     <a href="req_gen_upload.php?id=<?php echo $row['req_detail_id']; ?>" class="btn btn-info btn-sm"><span class="glyphicon glyphicon-edit"></span> รูปภาพ </a>
	     <?php echo $btnSentProduct  ?>
	  </td>
     </tr>
   <?php
    }
...
$('#find').click(()=>{
  ...
  window.location.href="req_list_supply.php?findDate="+find_date;
});
...
$('#find_all').click(()=>{
  window.location.href="req_list_supply.php";
});
